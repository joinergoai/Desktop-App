...
export class AppHeader extends LitElement {
    static properties = {
        ...
        showStopConfirmModal: { type: Boolean, state: true },
    };
    ...
    static styles = css`
        ...
        * {
        ...
        .modal-content::after {
            ...
            border-radius: 16px;
            padding: 1px;
            background: linear-gradient(169deg, rgba(255, 255, 255, 0.2) 0%, rgba(255, 255, 255, 0.08) 50%, rgba(255, 255, 255, 0.2) 100%);
        ...
        /* Compact confirmation styles for fitting within header */
    ...
    async handleConfirmStop() {
        if (window.require) {
            ...
            // Hide any visible feature windows
            ipcRenderer.send('hide-window', 'ask');
            ipcRenderer.send('hide-window', 'listen');
            
            // Close the session
            await ipcRenderer.invoke('close-session');
            
            // Clear the calendar event and deal info
            ipcRenderer.send('set-calendar-event', null);
            ipcRenderer.send('set-deal-info', null);
            
            // Navigate back to calendar selector using DOM event
            // Since we're in the same window as HeaderController
            window.dispatchEvent(new CustomEvent('navigate-to-calendar-selector'));
        }
    }

    render() {
        return html`
            <div class="header ${this.showStopConfirmModal ? 'confirmation-mode' : ''}" @mousedown=${this.handleMouseDown}>
                ${this.showStopConfirmModal ? html`
                    <div class="confirmation-container">
                        <span class="confirmation-text">End session?</span>
                        <div class="confirmation-buttons">
                            <button class="confirmation-button cancel" @click=${this.handleCancelStop}>
                                Cancel
                            </button>
                            <button class="confirmation-button confirm" @click=${this.handleConfirmStop}>
                                End
                            </button>
                        </div>
                    </div>
                ` : html`
                    <button 
                        ...
                        @click=${() => this.isSessionActive ? this.handleStopClick() : this.invoke('toggle-feature', 'listen')}
                    >
